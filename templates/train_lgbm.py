...
Copy to experiments/lgbm_v<N>/train.py and adapt params. Obeys the contract in common.py:
shared folds, canonical class order, standard OOF/test schema, one scoreboard row.

    python templates/train_lgbm.py            # or python experiments/lgbm_v1/train.py
"""
...
TAG = "lgbm_v1"
N_SEEDS = 1  # bump to 3-5 to seed-bag the final model (OOF stays aligned to the shared folds)
...
def main():
    ...
    for seed in range(N_SEEDS):
        ...
        for f in range(n_folds):
            ...
            if not va.any():
                continue
            dtr = lgb.Dataset(Xtr[tr], label=y_idx[tr], categorical_feature=cat_cols or "auto")
